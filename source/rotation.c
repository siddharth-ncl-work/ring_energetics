...
double *getDirectionality(char *files[],int start_frame,int end_frame,int step_size){
...
double *getRingRotation(char **files,int f1,int f2,int test){
  int i=0,j=0,isf1=0,isf2=0;
  double *f1com,*f2com,*avg_rpy,*v;//,rpy[3]={0,0,0};
...
double *getTrackRotation(char **files,int f1,int f2,int start_atom_no,int end_atom_no,int test){
  int i=0,j=0,isf1=0,isf2=0;
  double *f1com,*f2com,*avg_rpy,*v;//,rpy[3]={0,0,0};
  avg_rpy=(double *)malloc(sizeof(double)*3);
  for(i=0;i<3;i++)avg_rpy[i]=0;
  ...
  for(i=start_atom_no;i<=end_atom_no&&i<atoms;i++){
    v=getRPY(f1cords[i],f2cords[i]);
    for(j=0;j<3;j++){
      avg_rpy[j]+=v[j]/(end_atom_no-start_atom_no+1);
    }
...
double *getRPY(double *v1,double *v2){
  ...
  //theta=acos(getDotPro(v1,v2)/(getMag(v1)*getMag(v2))); (old formula of theta results might differ in 3rd/4th decimal places
  R=getR(s,theta);
...
/*
 d=sqrt((R[0][0]*R[0][0]+R[1][0]*R[1][0]+R[2][1]*R[2][1]+R[2][2]*R[2][2])/2);
  rpy[1]=atan2(-1*R[2][0],d);

  n=(R[2][1]*R[0][2]-R[2][2]*R[0][1]);
  if(n==0) rpy[2]=atan(0);
  else rpy[2]=atan2(n,(R[2][2]*R[1][1]-R[2][1]*R[1][2]));

  n=(R[1][0]*R[0][2]-R[0][0]*R[1][2]);
  if(n==0) rpy[0]=atan(0);
  else rpy[0]=atan2(n,(R[0][0]*R[1][1]-R[1][0]*R[0][1]));
  */
